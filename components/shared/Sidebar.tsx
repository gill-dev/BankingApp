  ...
  return (
    <aside className="sidebar">
      <div className="flex size-full flex-col gap-4">
        <Link href="/" passHref className="sidebar-logo">
        ...
        {data && (
          <div className="flex justify-center items-center  text-active-500">
            <h1 className="p-18-semibold">{data[0].accountType}</h1>
          </div>
        )}
          ...
          <ul className="sidebar-nav_elements">
            {navLinks.slice(0, 3).map((link) => {
              ...
              return (
                <li
                  key={link.route}
                  className={`sidebar-nav_element group ${isActive ? 'bg-active-500 text-white' : 'text-gray-700'}`}
                >
                  <Link href={link.route} passHref className="sidebar-link">
                      <Image
                        src={link.icon}
                        alt="logo"
                        width={24}
